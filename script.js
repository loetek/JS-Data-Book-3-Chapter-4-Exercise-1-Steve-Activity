...
// NOT NECESSARY JUST WANTED THE PRINT TO BE A LITTLE MORE UNIFORM. SO I CREATED A QUICK PRINT FUNCTION.
 const printLog = function (howMany, eventType){

  console.log(`There are ${howMany} registered ${eventType} events`);
...
// I then created a seperate function in order to log those numbers to the console.  
 ...
 for (let i = 0; i < githubData.length; i++){
   ...
   const indexEventsMacro = githubData[i].type;
   
      if ( indexEventsMacro === "PushEvent" ){

        pushEvent+=1;
        
      }
      
      if ( indexEventsMacro === "PushRequestEvent" ){
        
        pushReq+=1;
        
      }
      if ( indexEventsMacro === "PullEvent" ){
        
        pullEvent+=1;
        
      }
      if ( indexEventsMacro === "PullRequestEvent" ){
        
        pullReq+=1;
        
      }
      if ( indexEventsMacro === "CreateEvent" ){
        
        createEvent+=1;
        
      }
      if ( indexEventsMacro === "DeleteEvent" ){
        
        deleteEvent+=1;
        
      }
      else if ( indexEventsMacro === "IssueCommentEvent" ){
    ...
    printLog(pullReq, "Pull Request");
    printLog(createEvent, "Create");
    printLog(deleteEvent, "Delete");
